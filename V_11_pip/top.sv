...
// ********************** wires to excute stage ************************
		...
		// Data memory control signals
		    ...
		    wire       			W_Z_EX  ;         // read m[0]
    	    wire       			W_O_EX  ;         // read m[1]
...
// ********************** wires to Mem stage ************************
		...
		// Data memory control signals
		    ...
		    wire       			W_Z_MEM  ;         // read m[0]
    	    wire       			W_O_MEM  ;         // read m[1]
...
PC_Control_Unit pcCu(
	...
	// Hazard unit 
	 ...
	 (
		...
		.OPCODE_fetch(Opcode), //output after ID/EX
    ...
    regExecuteMemory EX_MEM (
		...
		always @(*) begin
			...
			endcase
				...
				4'b0001: Reg_B_forward = Alu_out_MEM_1;                    // From ALU output      (Previous instruction)
				4'b0011: Reg_B_forward = input_port_MEM;                   // From Input Port      (Previous instruction)
				4'b0100: Reg_B_forward= Imm_MEM;                          // From Immediate Value (Previous instruction)
				4'b1000: Reg_B_forward= out_WB;                           // From Data Memory     (Previous Previous instruction)
				4'b1001:Reg_B_forward= Alu_out_WB;                       // From ALU output      (Previous Previous instruction)
				4'b1011: Reg_B_forward = input_port_WB;                    // From Input Port	   (Previous Previous instruction)
				4'b1100: Reg_B_forward = Imm_WB;                           // From Immediate Value (Previous Previous instruction)
				default: Reg_B_forward = R_rb_EX ;                         // From Register File   (Normal case)
			endcase
	...
	DataMEM DM(
	    ...
	    .W_Z(W_Z_MEM),		// read m[0]
	    .W_O(W_O_MEM),		// read m[0]
   ...
   always @(posedge clk) begin
	...
	DM.mem[DM.Sp] = (Cu.Opcode[7:4]==4'h7 && Cu.Opcode[3:2]==2'b01 ) ? 8'h0 : DM.mem[DM.Sp];
