module Hazard_Unit (
    ...
    input  wire [1:0]  W_add_EX,         // write address for EX stage instruction (destination register)
...
always @(*) begin
    ...
    if (W_E_R_EX && (w_Data_S_R_EX == 3'd0)) begin
        if ((W_add_EX == R_ADD_A_ID && R_ADD_A_ID != 2'b11) ||
            (W_add_EX == R_ADD_B_ID && R_ADD_B_ID != 2'b11))
            STALL = 1'b1;
    end
end
