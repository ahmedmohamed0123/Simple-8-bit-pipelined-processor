...
(
   ...
   output reg [2:0] forward_A,            // to forward data for port A for ALU (ALU's A port's MUX select line)
   output reg [2:0] forward_B             // to forward data for port B for ALU (ALU's B port's MUX select line)
);
...
always @(*) begin 
   ...
   if (W_E_R_previous && (R_ADD_A_current != 2'b11) && (distination_prev==R_ADD_A_current) ) begin
      ...
      // Memory hazard (Dependency RAW) (L_type then arithmetic or logical LDD r1,r2(0) -> ADD r3,r1) (Will still have one stall cycle)
         3'd0: forward_A = 3'b010;
      // Execution hazard (Dependency RAW) (Arithmetic or logical then arithmetic or logical ADD r1.r2 -> ADD r3,r1)
       3'd1:  forward_A = 3'b001;
       //3'd4: forward_A=3'b011; // Immediate value  (L_type then arithmetic or logical LDI r1,r2(0) -> ADD r3,r1) (Will still have one stall cycle)
       3'd3: forward_A=3'b100; // RAW dependency  IN + Arithmetic
