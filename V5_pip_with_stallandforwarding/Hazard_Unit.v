...
always @(*) begin
   ...
   distination_prev=(W_add_S_R)? rb_add_EX :ra_add_EX;
    // ==============================
    // CONTROL HAZARDS → FLUSH
    // ==============================
    if (OPCODE_EX[7:4]== 4'd9) begin
        if ((ra_add_EX == 2'd0 && CCR[0]) ||  // JZ
            (ra_add_EX == 2'd1 && CCR[1]) ||  // JN
            (ra_add_EX == 2'd2 && CCR[2]) ||  // JC
            (ra_add_EX== 2'd3 && CCR[3]))    // JV
            FLUSH = 1'b1;
    end
    ...
    if (W_E_R_EX &&((w_Data_S_R_EX == 3'd0) ) ) begin
        if (((distination_prev==R_ADD_A_ID)&& R_ADD_A_ID != 2'b11) ||((distination_prev==R_ADD_B_ID)&& R_ADD_B_ID != 2'b11) )
            STALL = 1'b1;
    end
end
