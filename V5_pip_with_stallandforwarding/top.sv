...
PC_Control_Unit pcCu(
	...
	//Forward unit 
	 forward_unit  Fu 
	 (
		.W_E_R_previous(w_E_R_MEM), // output after EX/MEM stage
		.R_ADD_A_current(ra_EX), // output after ID/EX stage
		.R_ADD_B_current(rb_EX), // output after ID/EX stage
		.W_add_B_previous(rb_MEM), //output after EX/MEM stage
		.W_add_A_previous(ra_MEM), // output after EX/MEM stage
		.w_Data_S_R_previous(w_Data_S_R_MEM), // output after EX/MEM stage
		.W_add_S_R(w_Add_S_R_MEM),
	...
	assign ALU_A = (forward_a == 3'b00) ? R_ra_EX :(forward_a == 3'b001) ? Alu_out_MEM_1 :(forward_a==3'b010)? out_MEM : input_port_MEM;

    assign ALU_B = (forward_b == 3'b00) ? R_rb_EX :(forward_b == 3'b001) ? Alu_out_MEM_1 :(forward_b==3'b010)? out_MEM : input_port_MEM;
