...
PC_Control_Unit pcCu(
	...
	//Forward unit 
	 forward_unit  Fu 
	 (
		.W_E_R_previous(w_E_R_MEM), // output after ID/EX stage
		.R_ADD_A_current(ra_EX), // output after IF/ID stage
		.R_ADD_B_current(rb_EX), // output after IF/ID stage
		.W_add_B_previous(rb_MEM), // output after ID/EX stage
		.W_add_A_previous(ra_MEM), // output after ID/EX stage
		.w_Data_S_R_previous(w_Data_S_R_MEM), // output after ID/EX stage
	...
	assign ALU_A = (forward_a == 2'b00) ? R_ra_EX :(forward_a == 2'b01) ? Alu_out_MEM_1 : out_MEM;

    assign ALU_B = (forward_b == 2'b00) ? R_rb_EX :(forward_b == 2'b01) ? Alu_out_MEM_1 : out_MEM;
